    ...
    def set_data(self):
        ...
        if not os.path.exists('%sData/%s' %(self.data_path, self.name)):
            ...
            if self.rule == 'Dirichlet' or self.rule == 'Pathological':
                ...
                # print(cls_amount)
                clnt_x = [ np.zeros((client_data_list[client__], self.channels, self.height, self.width)).astype(np.float32) for client__ in range(self.n_client) ]
                clnt_y = [ np.zeros((client_data_list[client__], 1)).astype(np.int64) for client__ in range(self.n_client) ]
                ...
                while(np.sum(client_data_list)!=0):
                    ...
                    while True:
                        ...
                        clnt_x[curr_client][client_data_list[curr_client]] = train_x[idx_list[cls_label][cls_amount[cls_label]]]
                        clnt_y[curr_client][client_data_list[curr_client]] = train_y[idx_list[cls_label][cls_amount[cls_label]]]
                ...
                clnt_x = np.zeros((self.n_client, n_data_per_client, 3, 32, 32), dtype=np.float32)
                clnt_y = np.zeros((self.n_client, n_data_per_client, 1), dtype=np.float32)
                ...
                for client_idx_ in range(self.n_client):
                    clnt_x[client_idx_] = train_x[client_data_list_cum_sum[client_idx_]:client_data_list_cum_sum[client_idx_+1]]
                    clnt_y[client_idx_] = train_y[client_data_list_cum_sum[client_idx_]:client_data_list_cum_sum[client_idx_+1]]
            ...
            self.clnt_y = np.load('%sData/%s/clnt_y.npy' %(self.data_path, self.name), mmap_mode = 'r')
            self.n_client = len(self.clnt_x)

            self.tst_x  = np.load('%sData/%s/tst_x.npy'  %(self.data_path, self.name), mmap_mode = 'r')
            self.tst_y  = np.load('%sData/%s/tst_y.npy'  %(self.data_path, self.name), mmap_mode = 'r')
