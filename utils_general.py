...
def set_client_from_params(mdl, params):
    ...
    dict_state = copy.deepcopy(mdl.state_dict())
    idx = 0
    for name, param in mdl.state_dict().items():
        weights = param.data
        length = len(weights.reshape(-1))
        dict_state[name].copy_(torch.tensor(params[idx:idx+length].reshape(weights.shape)).to(device))
        idx += length
        
    mdl.load_state_dict(dict_state)
    return mdl
'''
...
def train_model(model, trn_x, trn_y, learning_rate, batch_size, epoch, print_per, weight_decay, dataset_name):
    ...
    for e in range(epoch):
        ...
        # Calculate overall acc vs loss in client
        if (e+1) % print_per == 0:
